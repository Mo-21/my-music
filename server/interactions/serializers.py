...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if Comment.objects.filter(user_id=user_id, content_type=content_type, object_id=object_id).exists():
            raise serializers.ValidationError(
                'You already commented this object'
            )

        comment = Comment.objects.create(user_id=user_id, **validated_data)
        return comment
